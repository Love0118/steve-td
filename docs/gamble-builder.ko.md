...
보상은 기존 업그레이드 서비스에서 한 번 확정하고 연출은 그 결과만 표시합니다. 결과 공개 전에는 같은 플레이어의 다른 도박 결제를 막아 연출이 겹치지 않도록 합니다. 결과를 공개하면 상세 결과를 채팅으로 보내고, 액션바는 2초간 결과를 유지한 뒤 일반 HUD로 돌아갑니다. 접속 종료·차원 이동·경기 종료에는 남은 연출 상태를 정리합니다.
