...
- `강화 점수 = 베팅액 × 패 점수`, `최대 체력 = 기본 최대 체력 + 강화 점수 × 3 / 170`. 이전 체력 증가량의 30%로 조정했으며 아래 체력은 **1,000다이아 베팅 기준**입니다. 작은 베팅에서는 증가량만 비례해서 줄어듭니다. 내부 체력은 소수로 유지하고 상세 표시는 반올림합니다.
...
특수능력 최소 베팅은 플러시 1,000 / 스트레이트 910 / 트리플·스트레이트 플러시 834입니다. 포커의 명목 추가 HP 기여는 평균 0.364970 HP/다이아입니다. 두 주사위 도박의 명목 평균은 HP 0.285948, 일반 공격력 0.028595, 마법 공격력 0.028595, 사거리 0.002859블록/다이아입니다. 포커는 HP에만 집중하는 대신 강한 사망 디버프를 갖습니다. 이 비교는 설치비, 보험, 전직, 능력치 하한·상한을 제외한 명목 수치이며 실제 전투 효율이 같다는 의미가 아닙니다.
