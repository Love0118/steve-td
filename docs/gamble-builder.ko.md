...
포커 테이블 모델은 Y축으로 90도 회전했습니다. 슬롯머신 세 단계와 도박꾼 계열은 각 라인의 웨이브 생성 위치를 바라봅니다. 도박꾼은 전투 중 기존 적 추적을 유지하고 대상이 없으면 웨이브 방향으로 돌아옵니다. 슬롯머신은 모든 본에 움직임 없는 idle 루프를 적용해 BIL이 실제 표시 방향을 계속 갱신하도록 합니다. 배율·텍스처·클릭 영역은 유지합니다.
