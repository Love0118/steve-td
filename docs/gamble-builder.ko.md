...
- 설치 비용 50, 기본 최대 체력 200, 어그로 50. 평타와 추격이 없습니다. 사거리·공격력 지원을 받아도 평타가 생기지 않습니다.
...
- `강화 점수 = 베팅액 × 패 점수`, `최대 체력 = 기본 최대 체력 + 강화 점수 × 3 / 170`. 일반 성공 패의 체력 증가량을 높였으며 아래 체력은 **1,000다이아 베팅 기준**입니다. 작은 베팅에서는 증가량만 비례해서 줄어듭니다. 내부 체력은 소수로 유지하고 상세 표시는 반올림합니다.
- 플러시 이상이면서 강화 점수 **50,000 이상**이면 사망 디버프를 얻습니다. 미달하면 체력만 얻습니다.
