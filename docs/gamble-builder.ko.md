...
보상은 기존 업그레이드 서비스에서 한 번 확정하고 연출은 그 결과만 표시합니다. 연출 중에도 다음 도박을 구매할 수 있습니다. 새 도박을 시작하면 이전 도박의 미공개 결과를 채팅으로 보내고 액션바를 새 연출로 교체합니다. 채팅은 공백 줄 → 폰트 그림과 상세 결과 줄 → 공백 줄 형식으로 표시하며, 이미 공개한 결과는 중복 전송하지 않습니다. 일반적으로는 결과 공개 시 상세 결과를 채팅으로 보내고, 액션바는 2초간 결과를 유지한 뒤 일반 HUD로 돌아갑니다. 강화 상한·지원 타워 조건·포커 테이블당 1회 제한은 계속 적용됩니다. 접속 종료·차원 이동·경기 종료에는 남은 연출 상태를 정리합니다.
