...
- `강화 점수 = 베팅액 × 패 점수`, `최대 체력 = 기본 최대 체력 + 강화 점수 × 3 / 170`. 일반 성공 패는 대실패와 차이가 나도록 Q 하이부터 상향했으며 아래 체력은 **1,000다이아 베팅 기준**입니다. 작은 베팅에서는 증가량만 비례해서 줄어듭니다. 내부 체력은 소수로 유지하고 상세 표시는 반올림합니다.
- 플러시 이상이면서 강화 점수 **50,000 이상**이면 강화할 때 사망 디버프를 **1~3개 무작위**로 얻습니다. 개수는 같은 확률로 추첨하고 공격력·공격 속도·방어력 감소 중 중복 없이 뽑습니다. 미달하면 체력만 얻습니다. 뽑힌 효과는 저장되며 라운드 변경·리로드·상태 복사로 다시 추첨하지 않습니다. 상세창에는 실제 보유 효과만 표시합니다.
...
| A 하이 | 약 798 | 없음 |
| 원페어 | 약 833~1,045 | 없음 |
| 플러시 | 약 1,062 | 무작위 1~3개 |
| 스트레이트 | 약 1,151 | 무작위 1~3개 |
| 트리플·스트레이트 플러시 | 약 1,239 | 무작위 1~3개 |
...
| 플러시 | 4.9593% | 50 | 무작위 1~3개 |
| 스트레이트 | 3.2579% | 55 | 무작위 1~3개 |
| 트리플 | 0.2353% | 60 | 무작위 1~3개 |
| 스트레이트 플러시 | 0.2172% | 60 | 무작위 1~3개 |
...
특수능력 최소 베팅은 플러시 1,000 / 스트레이트 910 / 트리플·스트레이트 플러시 834입니다. 포커의 명목 추가 HP 기여는 평균 0.470316 HP/다이아입니다. 두 주사위 도박의 명목 평균은 HP 0.285948, 일반 공격력 0.028595, 마법 공격력 0.028595, 사거리 0.002859블록/다이아입니다. 포커는 HP에만 집중하는 대신 강한 사망 디버프를 갖습니다. 이 비교는 설치비, 보험, 전직, 능력치 하한·상한을 제외한 명목 수치이며 실제 전투 효율이 같다는 의미가 아닙니다.
...
보상은 기존 업그레이드 서비스에서 한 번 확정하고 연출은 그 결과만 표시합니다. 연출 중에도 다음 도박을 구매할 수 있습니다. 새 도박을 시작하면 이전 도박의 미공개 결과를 채팅으로 보내고 액션바를 새 연출로 교체합니다. 채팅은 공백 줄 → 결과 폰트 그림과 능력치 변화량 줄 → 공백 줄 형식으로 표시하며, 이미 공개한 결과는 중복 전송하지 않습니다. 일반적으로는 결과 공개 시 상세 결과를 채팅으로 보내고, 액션바는 2초간 결과를 유지한 뒤 일반 HUD로 돌아갑니다. 강화 상한·지원 타워 조건·포커 테이블당 1회 제한은 계속 적용됩니다. 접속 종료·차원 이동·경기 종료에는 남은 연출 상태를 정리합니다.
...
포커 테이블 모델은 원래 세로 배치로 복원했습니다. 슬롯머신 세 단계와 도박꾼 계열은 각 라인의 웨이브 생성 위치를 바라봅니다. 도박꾼은 전투 중 기존 적 추적을 유지하고 대상이 없으면 웨이브 방향으로 돌아옵니다. 배율·텍스처·클릭 영역은 유지합니다.
